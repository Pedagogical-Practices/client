  ...
  <v-container>
    <v-row>
      ...
      <v-col class="text-right">
        <v-btn color="primary" @click="openCreateModal">Crear Institución</v-btn>
      </v-col>
    </v-row>
    ...
    <v-dialog v-model="deleteDialog" persistent max-width="400px">
      <v-card>
        ...
        <v-card-text
          >¿Estás seguro de que quieres eliminar esta institución? Esta acción no
...
const saveInstitution = async () => {
  try {
    if (isEditing.value) {
      await institutionStore.updateInstitution(editableInstitution.value as Institution);
      showSnackbar("¡Institución actualizada exitosamente!", "success");
    } else {
      await institutionStore.createInstitution(editableInstitution.value as Institution);
      showSnackbar("¡Institución creada exitosamente!", "success");
    }
    closeModal();
  } catch (error: any) {
    console.error("Error al guardar institución:", error);
    showSnackbar(`Error al guardar institución: ${error.message || "Error desconocido"}`, "error");
  }
};
...
const deleteInstitutionConfirmed = async () => {
  try {
    ...
    showSnackbar(`Error al eliminar institución: ${error.message || "Error desconocido"}`, "error");
  } finally {
