    ...
    <v-row v-else-if="group">
      <v-col cols="12">
        <v-card>
          ...
          <v-card-subtitle>{{ group.name }} - Periodo: {{ group.period }}</v-card-subtitle>
          <v-card-text>
            <p><strong>Tutor:</strong> {{ group.tutor.firstName }} {{ group.tutor.lastName }}</p>
            <p><strong>Instituciones:</strong> {{ group.institutions.map(i => i.name).join(', ') }}</p>
          </v-card-text>
        </v-card>
      </v-col>
      <v-col cols="12">
