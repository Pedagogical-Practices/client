  ...
  <div>
    ...
    <v-radio-group v-model="selectedValue">
      <div v-for="option in options" :key="option.value" class="radio-item">
        <v-radio :label="option.label" :value="option.value" hide-details density="compact"></v-radio>
        <v-text-field
...
const selectedValue = computed({
  get() {
    if (!props.modelValue || typeof props.modelValue !== 'object' || Object.keys(props.modelValue).length === 0) {
      return null;
    }
...
const updateOtherText = (newText: string) => {
  const otherOption = props.options.find(opt => opt.isOtherOption && opt.value === selectedValue.value);
  if (otherOption) {
